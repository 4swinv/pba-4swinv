...
void collide_particle_ball(
  ...
  const Eigen::Vector2f impulse = particle_mass * p.velo - ball_mass * ball_velo;
  p.velo += - p.velo.dot(plane_norm)* plane_norm + ( - impulse + ball_mass * (p.velo - ball_velo) ).dot(plane_norm) * plane_norm / (particle_mass - ball_mass);
  ball_velo +=  ( - impulse + particle_mass * (p.velo - ball_velo) ).dot(plane_norm) * plane_norm / (particle_mass - ball_mass);
}
